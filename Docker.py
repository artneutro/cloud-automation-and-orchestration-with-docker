...
class Docker:
    ...
    def stop_container ( self ):
        #
        # This function request a containr ID and stop the container.
        # 
        try:
            self.list_containers()
            container_id = input("Please insert the container ID: ")
    ...
    def python_container ( self ):
        #
        # This function creates a container that runs a python script provided by the user.
        # 
        try:
            # Request the python file name
            while True:
                files = self.list_files()
                file_name = input("Please insert the python file name: ")
                if file_name in files:
                    break
                else:
                    self.error.not_valid_value(file_name)
            # Request the python version
            while True:
                print("Please insert the python version. Possible options: 2 or 3 ")
                print("2 ==> 2.7")
                python_version = input("3 ==> 3.8 : ")
                if str(python_version) in ["2","3"]:
                    break
                else:
                    self.error.not_valid_value(file_name)
            # Request image name
            image_name = input("Please insert the image name: ")
